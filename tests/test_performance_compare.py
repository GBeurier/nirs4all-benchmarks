...
def test_frozen_plan_pins_selected_candidates_and_native_predictor() -> None:
    ...
    assert {name: value["commit_sha"] for name, value in plan["candidates"].items()} == {
        ...
        "core": "550cb8c80708e88ac7ebbc880acb4b82d8531632",
        "python": "53a0acb964bff86dc67002763d8e9b850336731f",
        "studio": "86d5e5033d62240815e532038b6e769b14b25c2b",
        "web": "051bf636d7c1729087e5d40061b18bd690cd33b7",
    }
    assert plan["selection_snapshot"] == {
        "selection_authority": "explicit final-head qualification request",
        "governance_source": (
            "docs/contracts/release/migration-work-ledger.yaml "
            "(pending projection of this local evidence)"
        ),
        "governance_benchmark_parent_sha": "9aab2e13513b4e0a5a699a11ebc70f2bd00f10fb",
        "observed_at": "2026-09-03",
        "selection_scope": (
            "current four product heads; other supporting candidates retain their prior selection"
        ),
        "status": "locally_qualified_synthetic_no_go",
        "four_runtime_execution": "passed_local_synthetic_current_heads",
        "release_gate_status": "no_go",
        "release_eligible": False,
        "historical_report_policy": (
            "The checked-in report measures the current four product heads on the deterministic "
            "Archive V2 workload; older reports remain attributable through git history."
        ),
        "holds": [
            "representative_user_corpus_missing",
            "sustained_soak_not_run",
            "external_release_matrices",
            "frozen_performance_budgets",
            "complete_governance_projection_not_staged",
    ...
    assert {
        ...
        for name in ("methods", "dag_ml", "core", "python", "studio", "web")
    } == {
    ...
    assert plan["local_smoke_hold"]["status"] == "passed_local_synthetic_current_head_no_go"
...
def test_web_handoff_contains_archive_matrix_predictor_and_web_candidate(tmp_path: Path) -> None:
    ...
    assert payload["web_candidate"]["commit_sha"] == "051bf636d7c1729087e5d40061b18bd690cd33b7"
...
def test_checked_in_four_runtime_report_attests_current_heads_without_release_claim() -> None:
    ...
    assert report["evidence_kind"] == "local_synthetic_current_head"
    assert report["overall_disposition"] == "passed"
    assert report["release_eligible"] is False
    assert "representative_user_corpus_missing" in report["release_eligibility_holds"]
    assert "sustained_soak_not_run" in report["release_eligibility_holds"]
